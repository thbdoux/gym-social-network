// app/(app)/programs/[programId]/workouts/[workoutId].tsx
...
// Shared components
import InstanceExerciseManager from './InstanceExerciseManager';
import { formatRestTime } from '../../workout/formatters';

export default function ProgramWorkoutDetailScreen() {
  // Get program ID and workout ID from route params
  const { programId, workoutId } = useLocalSearchParams();
  const programIdNum = typeof programId === 'string' ? parseInt(programId, 10) : 0;
  const workoutIdNum = typeof workoutId === 'string' ? parseInt(workoutId, 10) : 0;
  ...
  const { user } = useAuth();
  const { t } = useLanguage();
  const { data: program, isLoading: isProgramLoading } = useProgram(programIdNum);
  const { 
    data: workout, 
    isLoading: isWorkoutLoading, 
    refetch 
  } = useProgramWorkout(programIdNum, workoutIdNum);
  const { mutateAsync: updateWorkout } = useUpdateProgramWorkout();
  const { mutateAsync: removeWorkout } = useRemoveWorkoutFromProgram();
  
  const isLoading = isProgramLoading || isWorkoutLoading;
  ...
  useEffect(() => {
    if (workout) {
      ...
      setWorkoutWeekday(workout.preferred_weekday || 0);
    }
  }, [workout]);
  ...
  const handleOptionsMenu = () => {
    Alert.alert(
      ...
      [
        ...
        {
          text: t('weekday'),
          onPress: () => handleEditWeekday()
        },
        {
          text: t('delete'),
          style: 'destructive',
          onPress: handleRemoveWorkoutFromProgram
        },
        {
          text: t('cancel'),
          style: 'cancel'
  ...
  // Handle editing workout weekday
  ...
  const handleSaveWorkoutField = async (field, value) => {
    try {
      // First, make sure we have the current workout data with exercises
      const currentWorkoutData = await refetch();
      const currentWorkout = currentWorkoutData.data;
  
      // Create updates with the changed field and preserve exercises
      const updates = {
        [field]: value,
        exercises: currentWorkout.exercises // Include current exercises
      };
  ...
  const handleRemoveWorkoutFromProgram = () => {
    Alert.alert(
      t('remove_workout'),
      t('confirm_remove_workout_from_program'),
      [
